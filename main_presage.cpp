...
// Compute median for robust vitals prediction (Presage SDK enhancement)
...
// Calculate vitals summary statistics (enhanced with median for Presage SDK)
json calculate_vitals_summary() {
    ...
    for (const auto& reading : all_vitals_readings) {
        if (reading.contains("heart_rate_bpm") && reading["heart_rate_bpm"].is_number()) {
            float hr = reading["heart_rate_bpm"];
            if (hr >= 40.0f && hr <= 200.0f) heart_rates.push_back(hr);
        }
        if (reading.contains("breathing_rate_bpm") && reading["breathing_rate_bpm"].is_number()) {
            float br = reading["breathing_rate_bpm"];
            if (br >= 0.0f && br <= 60.0f) breathing_rates.push_back(br);
        }
    }
    
    auto calc_stats = [](const std::vector<float>& values) -> json {
        if (values.empty()) return json::object();
        float sum = 0.0f, min_val = values[0], max_val = values[0];
        for (float v : values) {
            sum += v;
        ...
        float med = compute_median(sorted);
        return {
            {"avg", sum / values.size()},
            {"median", med},
            {"min", min_val},
    ...
    return {
        ...
        {"readings_count", static_cast<int>(all_vitals_readings.size())},
...
void run_camera_test(const std::string& api_key) {
    ...
    try {
        ...
        // Metrics callback - store all readings from Presage SmartSpectra SDK (enhanced prediction)
        auto status = container->SetOnCoreMetricsOutput(
            [](const presage::physiology::MetricsBuffer& metrics, int64_t timestamp) {
                ...
                // Heart rate: prefer strict() when available (more reliable), else rate()
                float pulse = 0.0f;
                if (metrics.pulse().strict().has_value()) {
                    pulse = metrics.pulse().strict().value();
                }
                if (pulse <= 0.0f && !metrics.pulse().rate().empty()) {
                ...
                // Breathing rate from Presage SDK
                if (!metrics.breathing().rate().empty()) {
                    float breathing = metrics.breathing().rate().rbegin()->value();
                    reading["breathing_rate_bpm"] = breathing;
                }
                if (metrics.breathing().strict().has_value()) {
                    reading["breathing_rate_bpm"] = metrics.breathing().strict().value();
                }
                
                if (reading.contains("heart_rate_bpm") || reading.contains("breathing_rate_bpm")) {
...
int main(int argc, char** argv) {
    ...
    // Create HTTP server
    httplib::Server svr;
    svr.set_payload_max_length(500 * 1024 * 1024); // 500MB max upload
